    ...
    for N in range(nplanemax):
        ...
        for ig3 in range(-N, N + 1):
            for ig2 in range(-(N - abs(ig3)), N - abs(ig3) + 1):
                for ig1 in set([-(N - abs(ig3) - abs(ig2)),
                                N - abs(ig3) - abs(ig2)]):
                    igp = (ig1, ig2, ig3)
...
#    print (len(igall),len(set(igall)))
    if nplane < np.Inf:
        if spinor:
            if 2 * ncnt != nplane:
                raise RuntimeError(
                    '*** error - computed 2*ncnt={0} != input nplane={1}'.format(2 * ncnt, nplane))
        else:
            if ncnt != nplane:
                raise RuntimeError(
                    '*** error - computed ncnt={0} != input nplane={1}'.format(ncnt, nplane))
...
#    print ("igall1=",igall1)
    igallsrt = np.argsort(
        (igall1[:, 2] * ng[1] + igall1[:, 1]) * ng[0] + igall1[:, 0])
#    print (igallsrt)
...
#    print (igall)
    ...
    Eg = Eg[srt]
    igall = igall[srt, :].T
    wall = [0] + list(np.where(Eg[1:] - Eg[:-1] > thresh)
                      [0] + 1) + [igall.shape[1]]
...
def symm_eigenvalues(
        ...
        spinor=True):
    npw1 = igall.shape[1]
    multZ = np.exp(-1.j * (2 * np.pi * \
                   np.linalg.inv(A).dot(T).dot(igall[:3, :] + K[:, None])))
