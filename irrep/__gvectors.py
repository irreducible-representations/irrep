...
def symm_eigenvalues(K,RecLattice,WF,igall,A=np.eye(3),S=np.eye(2),T=np.zeros(3),spinor=True):
    ...
    if spinor:
        return  np.dot(  WF[:,igrot].conj()*WF[:,:npw1]*S[0,0] + WF[:,igrot+npw1].conj()*WF[:,npw1:]*S[1,1]
              +WF[:,igrot].conj()*WF[:,npw1:]*S[0,1]  + WF[:,igrot+npw1].conj()*WF[:,:npw1]*S[1,0],  multZ)
    else :
        return  np.dot(  WF[:,igrot].conj()*WF[:,:],  multZ)



def symm_matrix(K,RecLattice,WF,igall,A=np.eye(3),S=np.eye(2),T=np.zeros(3),spinor=True):
# computes the matrix S_mn = <Psi_m|T|Psi_n>
    npw1=igall.shape[1]
    multZ=np.exp(-1.j*(2*np.pi*A.dot(T).dot(igall[:3,:]+K[:,None])))
    igrot=transformed_g(K,igall,RecLattice,A)
    if spinor:
