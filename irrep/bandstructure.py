...
class BandStructure:
    ...
    def __init__(
        ...
        code = code.lower()
        if code == "vasp":
            self.__init_vasp(fWAV, fPOS, Ecut, IBstart, IBend,
                             kplist, spinor, EF=EF, onlysym=onlysym)
        elif code == "abinit":
            self.__init_abinit(fWFK, Ecut, IBstart, IBend,
                               kplist, EF=EF, onlysym=onlysym)
        elif code == "espresso":
            self.__init_espresso(prefix, Ecut, IBstart,
                                 IBend, kplist, EF=EF, onlysym=onlysym)
        elif code == "wannier90":
            self.__init_wannier(prefix, Ecut, IBstart, IBend,
                                kplist, EF=EF, onlysym=onlysym)
        else:
...
#        print ("kplist",kplist)
    ...
    def __init_wannier(
        ...
        fwin = [[s.strip() for s in split(l)]
                for l in fwin if len(l) > 0 and l[0] not in ('!', '#')]
        ...
        def get_param(key, tp, default=None, join=False):
            ...
            if len(i) == 0:
                if default is None:
                    raise RuntimeError(
                        "parameter {} was not found in {}.win".format(
                            key, prefix))
                else:
                    return default
            if len(i) > 1:
...
#        print ("nbands=",NBin)
        ...
        if EF is None:
            try:
                self.efermi = get_param(
                    "fermi_energy", float, 0.) if EF is None else EF
            except Exception as err:
                print(
                    "WARNING : fermi_energy not found.Setting as zero : `{}`".format(err))
                self.efermi = 0
        else:
            self.efermi = EF
            ...
            if l[0].startswith("begin"):
                if l[1] == "unit_cell_cart":
                    if self.Lattice is not None:
                        raise RuntimeError(
                            "'begin unit_cell_cart' found more then once  in {}.win".format(prefix))
                    j = 0
                    l1 = next(iterwin)
                    if l1[0] in ("bohr", "ang"):
                        units = l1[0]
                    ...
                    if kpred is not None:
                        raise RuntimeError(
                            "'begin kpoints' found more then once  in {}.win".format(prefix))
                    kpred = np.array([next(iterwin)[:3]
                                      for i in range(NK)], dtype=float)
#                    kpred=np.array([kpred[j].split()[:3] for j in kplist],dtype=float)
                    check_end("kpoints")
                elif l[1].startswith("atoms_"):
                    if l[1][6:10] not in ("cart", "frac"):
                        raise RuntimeError(
                            "unrecognised block :  '{}' ".format(l[0]))
                    if found_atoms:
                        raise RuntimeError(
                            "'begin atoms_***' found more then once  in {}.win".format(prefix))
                    found_atoms = True
                    xred = []
                    nameat = []
                    while True:
...
#        print ("lattice vectors:\n",self.Lattice)
        ...
        try:
            ...
            if not np.all(ik == np.arange(1, NK + 1)
                          [:, None] * np.ones(NBin, dtype=int)[None, :]):
                raise RuntimeError("wrong k-point indices")
            ib = np.array(eigenval[:, 0]).reshape(NK, NBin)
            if not np.all(ib == np.arange(1, NBin + 1)
                          [None, :] * np.ones(NK, dtype=int)[:, None]):
...
#        print ("eigenvalues are : ",eigenval)
        self.kpoints = [Kpoint(ik - 1,
                               ...
                               kpt=kpred[ik - 1]) for ik in kplist]
    ...
    def __init_espresso(
        ...
        atnames = [sp.attrib['name']
                   for sp in inp.find('atomic_species').findall('species')]
...
#        print ('spinor=',self.spinor)
        ...
        self.Lattice = BOHR * np.array([struct.find("cell").find(
            'a{}'.format(i + 1)).text.strip().split() for i in range(3)], dtype=float)
        xcart = []
        typat = []
        for at in struct.find("atomic_positions").findall("atom"):
            typat.append(atnumbers[at.attrib["name"]])
            xcart.append(at.text.split())
        xred = (np.array(xcart, dtype=float) *
                BOHR).dot(np.linalg.inv(self.Lattice))
#        print ("xred=",xred)
        self.spacegroup = SpaceGroup(
            cell=(self.Lattice, xred, typat), spinor=self.spinor)
        if onlysym:
...
#        exit()
    ...
    def write_characters(
        ...
        if kpnames is not None and refUC is not None:
            for kpname, KP in zip(kpnames, self.kpoints):
                ...
                ninv, low, up = KP.write_characters(
                    degen_thresh, irreptable=irreps, symmetries=symmetries, preline=preline, efermi=self.efermi, plotFile=pFile, kpl=kpline)
                NBANDINV += ninv
                GAP = min(GAP, up - low)
                Up = min(Up, up)
                Low = max(Low, low)
            ...
            for KP, kpl in zip(self.kpoints, kpline):
                ninv, low, up = KP.write_characters(
                    degen_thresh, symmetries=symmetries, preline=preline, efermi=self.efermi, plotFile=pFile, kpl=kpl)
                NBANDINV += ninv
                GAP = min(GAP, up - low)
                Up = min(Up, up)
                Low = max(Low, low)
        ...
        if self.spinor:
            print("number of inversions-odd Kramers pairs IN THE LISTED KPOINTS: ",
                  int(NBANDINV / 2), "  Z4= ", int(NBANDINV / 2) % 4)
        else:
...
#        print ("Total number of inversion-odd Kramers pairs IN THE LISTED KPOINTS: ",NBANDINV,"  Z4= ",NBANDINV%4)
    ...
    def write_trace(
        ...
        for KP in self.kpoints:
            f.write("   ".join("{0:10.6f}".format(x) for x in (
                refUC.dot(KP.K) if refUC is not None else KP.K)) + "\n")
        for KP in self.kpoints:
            f.write(KP.write_trace(degen_thresh,
                                   symmetries=symmetries, efermi=self.efermi))
    ...
    def Separate(self, isymop, degen_thresh=1e-5, groupKramers=True):
        ...
        kpseparated = [
            kp.Separate(
                ...
                groupKramers=groupKramers) for kp in self.kpoints]
        allvalues = np.array(sum((list(kps.keys())
...
#        allvalues=np.array(allavalues)
        if groupKramers:
            ...
            borders = np.hstack(([0], np.where(
                abs(allvalues[1:] - allvalues[:-1]) > 0.01)[0] + 1, [len(allvalues)]))
#            nv=len(allvalues)
            if len(borders) > 2:
                allvalues = set([allvalues[b1:b2].mean()
            ...
            borders = np.where(
                abs(allvalues - np.roll(allvalues, 1)) > 0.01)[0]
            nv = len(allvalues)
            if len(borders) > 0:
...
#                    print ("v,vk",v,vk[np.argmin(np.abs(v-vk))])
    ...
    def zakphase(self):
        overlaps = [x.overlap(y) for x, y in zip(
            self.kpoints, self.kpoints[1:] + [self.kpoints[0]])]
        print("overlaps")
        for O in overlaps:
...
#        overlaps.append(self.kpoints[-1].overlap(self.kpoints[0],g=np.array( (self.kpoints[-1].K-self.kpoints[0].K).round(),dtype=int )  )  )
        ...
        z = np.angle([[la.det(O[:n, :n]) for n in range(1, nmax + 1)]
                      for O in overlaps]).sum(axis=0) % (2 * np.pi)
#        print (np.array([k.Energy[1:] for k in self.kpoints] ))
#        print (np.min([k.Energy[1:] for k in self.kpoints],axis=0) )
        emin = np.hstack((np.min([k.Energy[1:nmax]
                                  for k in self.kpoints], axis=0), [np.Inf]))
        emax = np.max([k.Energy[:nmax] for k in self.kpoints], axis=0)
        locgap = np.hstack((np.min(
            [k.Energy[1:nmax] - k.Energy[0:nmax - 1] for k in self.kpoints], axis=0), [np.Inf]))
    ...
    def wcc(self):
        overlaps = [x.overlap(y) for x, y in zip(
            self.kpoints, self.kpoints[1:] + [self.kpoints[0]])]
        nmax = np.min([o.shape for o in overlaps])
        wilson = functools.reduce(np.dot, [functools.reduce(
            np.dot, np.linalg.svd(O)[0:3:2]) for O in overlaps])
            ...
            return "\n\n\n".join(
                "\n".join(
                    ("{0:8.4f}   {1:8.4f}  ".format(
                        ...
                        e) +
                        "  ".join(
                        "{0:8.4f}".format(l) for l in L)) for k,
                    e,
                    L in zip(
                        kpline,
                        E,
    ...
    def write_trace_all(
        ...
        f.write(
            "\n".join(
                ("#" +
                 l) for l in self.spacegroup.write_trace(
                    refUC=refUC,
                    shiftUC=shiftUC).split("\n")) +
            "\n\n")
        for KP, KPL in zip(self.kpoints, kpline):
            f.write(
                KP.write_trace_all(
