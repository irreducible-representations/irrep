...
#        self.n=np.arange(IBstart,IBend)+1
        ...
        if code.lower() == "vasp":
            ...
            self.WF, self.ig = self.__init_abinit(
                fWFK, ik, NBin, IBstart, IBend, Ecut, Ecut0, kpt=kpt, npw_=npw_, flag=flag, usepaw=usepaw)
        ...
        self.WF /= (np.sqrt(np.abs(np.einsum("ij,ij->i",
                                             self.WF.conj(), self.WF)))).reshape(self.Nband, 1)
        self.SG = SG
...
#        print ("g_shift={}".format(g_shift))
        selectG = np.array(np.where([is_round(
            dg, prec=1e-4) for dg in (self.ig[:3].T.dot(np.linalg.inv(supercell.T)) - g_shift)])[0])
#        print ("unfolding {} to {}, selecting {} of {} g-vectors \n".format(self.K,kptPBZ,len(selectG),self.ig.shape[1],selectG,self.ig.T))
        ...
        for b1, b2, E, matrices in self.get_rho_spin(degen_thresh):
            proj = np.array([[WF[i].conj().dot(WF[j])
                              for j in range(b1, b2)] for i in range(b1, b2)])
            result.append([E, ] +
                          [np.trace(proj.dot(M)).real for M in matrices])
    ...
    def __eval_rho_spin(self, degen_thresh):
        borders = np.hstack([[0], np.where(
            self.Energy[1:] - self.Energy[:-1] > degen_thresh)[0] + 1, [self.Nband]])
        result = []
...
#        print ("symmetry matrix \n",shortS)
        ...
        def short(A):
            return "".join("   ".join(
                "{0:+5.2f} {1:+5.2f}".format(x.real, x.imag) for x in a) + "\n" for a in A)
...
#        print ("Eloc:\n",Eloc)
        ...
        if np.abs((np.abs(w) - 1.)).max() > 3e-1:
            raise RuntimeError(
                " some eigenvalues are not unitary :{0} ".format(w))
        w /= np.abs(w)
        nb = len(w)
        ...
        if groupKramers:
            ...
            if len(borders) > 0:
                ...
                v1 = v
                subspaces[w.mean()] = self.copy_sub(
                    E=Eloc, WF=v1.T.dot(self.WF))
        else:
    ...
    #    print (r)
        ...
        ig = calc_gvectors(
            ...
            Ecut,
            spinor=self.spinor)
        selectG = np.hstack((ig[3], ig[3] + int(npw / 2))
                            ) if self.spinor else ig[3]
        WF = np.array([WCF.record(3 + ik * (NBin + 1) + ib, npw,
                                  np.complex64)[selectG] for ib in range(IBstart, IBend)])
        return WF, ig
        ...
        if usepaw == 0:
            assert np.max(
                np.abs(
                    ...
                    np.eye(
                        IBend -
                        IBstart))) < 1e-10  # check orthonormality
    ...
    def __init_wannier(
        ...
        ig = calc_gvectors(self.K, self.RecLattice, Ecut,
                           spinor=self.spinor, nplanemax=np.max([ngx, ngy, ngz]) // 2)
        ...
        def _readWF_1(skip=False):
            ...
            cg_tmp = (cg_tmp[0::2] + 1.j * cg_tmp[1::2]
                      ).reshape((ngx, ngy, ngz), order='F')
...
#        print ("k-point {0}: {1}".format(ik,self.K ))
    ...
    def __sortIG(self, kg, kpt, CG, RecLattice, Ecut0, Ecut, thresh=1e-4):
        ...
        igall[:3, :] = kg[srt].T
        igall[3, :] = srt
        wall = [0] + list(np.where(eKG[1:] - eKG[:-1] > thresh)
                          [0] + 1) + [igall.shape[1]]
    ...
    def write_characters(
        ...
        if abs(Nirrep - round(Nirrep)) > 1e-2:
            print(
                "WARNING - non-integer number of irreps : {0}".format(Nirrep))
        Nirrep = int(round(Nirrep))
        char = np.array([char[:, start:end].sum(axis=1)
...
#        print(" char ",char.shape,"\n",char)
        ...
        E = np.array([self.Energy[start:end].mean()
                      for start, end in zip(borders, borders[1:])])
        dim = np.array(
            [end - start for start, end in zip(borders, borders[1:])])
        if irreptable is None:
            irreps = ["None"] * (len(borders) - 1)
        else:
            try:
                irreps = [{ir: np.array([irreptable[ir][sym.ind] for sym in self.symmetries]).dot(
                    ch.conj()) / len(ch) for ir in irreptable} for ch in char]
            ...
            irreps = [", ".join(ir +
                                ...
                                ("{0:+.5f}i".format(irr[ir].imag) if abs(irr[ir].imag) > 1e-4 else "") +
                                ")" for ir in irr if abs(irr[ir]) > 1e-3) for irr in irreps]
#            irreps=[ "None" ]*(len(borders)-1)
        ...
        print(
            "   Energy  | multiplicity |{0} irreps {0}| sym. operations  ".format(s2))
        print("           |              |{0}        {0}| ".format(
            s2), " ".join(s1 + "{0:4d}    ".format(i) + s1 for i in sorted(sym)))
            ...
            plotFile.write(
                ("\n".join(
                    ...
                    " ".join(
                        ...
                        (
                            "{0:+7.4f}j".format(
                                c.imag) if writeimaginary else "") for c in ch) for e,
                    d,
                    ch in zip(
                        E,
                        dim,
        ...
        isyminv = None
        for s in sym:
            if sum(abs(sym[s].translation)
                   ) < 1e-6 and abs(sym[s].rotation + np.eye(3)).sum() < 1e-6:
                isyminv = s
        if isyminv is None:
            print("no inversion")
            NBANDINV = 0
            ...
            NBANDINV = int(
                round(sum(1 - self.symmetries[sym[isyminv]].real) / 2))
            if self.spinor:
    ...
    def write_trace_all(
        ...
        borders = np.hstack([[0], np.where(
            self.Energy[1:] - self.Energy[:-1] > degen_thresh)[0] + 1, [self.Nband]])
        char = {i: np.array([char0[i][start:end].sum() for start, end in zip(
            borders, borders[1:])]) for i in char0}
        E = np.array([self.Energy[start:end].mean()
                      for start, end in zip(borders, borders[1:])])
        dim = np.array(
            [end - start for start, end in zip(borders, borders[1:])])
        IB = np.cumsum(np.hstack(([0], dim[:-1]))) + 1
        res = "\n".join(
            preline + (" {ib:8d}  {d:8d}   {E:8.4f} ").format(
                E=e - efermi, d=d, ib=ib) + "     ".join(
                ("{0:10.6f} {1:10.6f}".format(
                    char[i][j].real, char[i][j].imag) if i in char else (
                    " " * 7 + "X" * 3 + " " * 8 + "X" * 3)) for i in range(
                        1, len(
                            self.SG.symmetries) + 1)) for e, d, ib, j in zip(
                                E, dim, IB, np.arange(
                                    len(dim)))) + "\n"
        return res
...
#        print (igsize)
        ...
        for s in ([0, 1] if self.spinor else [0]):
            ...
            for i, ig in enumerate(self.ig.T):
                WF1[:, ig[0] - igmin[0], ig[1] - igmin[1], ig[2] -
                    igmin[2]] = self.WF[:, i + s * self.ig.shape[1]]
            for i, ig in enumerate(other.ig[:3].T - g[None, :]):
                WF2[:, ig[0] - igmin[0], ig[1] - igmin[1], ig[2] -
                    igmin[2]] = other.WF[:, i + s * other.ig.shape[1]]
...
#        return np.einsum("ma,na->mn",self.WF.conj(),other.WF)
    ...
    def getloc1(self, loc):
        ...
        print("grid:", grid)
        loc_grid = loc(grid[0][:, None, None], grid[1]
                       [None, :, None], grid[2][None, None, :])
...
#            print ("wfsum",WF1.sum()," shape ",WF1.shape,loc_grid.shape)
            res += np.array([np.sum(np.abs(np.fft.ifftn(WF1[ib]))
                                    ** 2 * loc_grid).real for ib in range(self.Nband)])
